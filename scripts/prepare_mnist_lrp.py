...
path_to_model = MODELS_DIR+'eval/mnist'
...
if os.path.exists(path_to_model):
    ...
    victim_model = mnist_cnn(input_shape=(32,32,1), num_of_classes=10)
    victim_model.compile(optimizer=optimizer, loss=loss_object, metrics=['accuracy'])
...
path_to_model = MODELS_DIR+'victim/mnist'
...
if os.path.exists(AUX_DIR+'mnist_lrp.hkl'):
    print("(pre-generated xai exists!)")
    print("... loading pre-generated xai ...")
    xai = hkl.load(AUX_DIR+'mnist_lrp.hkl')
else:
    ...
    optimizer = tf.keras.optimizers.Adam(0.0001, beta_1=0.5, beta_2=0.999)
    victim_model = tf.keras.models.load_model(path_to_model, compile=False)
    victim_model.compile(optimizer=optimizer, loss=loss_object, metrics=['accuracy'], experimental_run_tf_function=False)
    
    
    
    lrps = LRP(victim_model, pool_type='avg').generate_batched(imgs, return_as_ds=False, batch_size=256)
    hkl.dump(lrps, AUX_DIR+'mnist_lrp.hkl', mode='w')