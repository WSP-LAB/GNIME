...
victim_model = tf.keras.models.load_model(MODELS_DIR+'victim/cifar10', compile=False)
...
ckpt_dir = MODELS_DIR+'def/cifar10_grad/'
os.makedirs(ckpt_dir, exist_ok=True)
fig_dir = FIGURES_DIR+'def/cifar10_grad/'
os.makedirs(fig_dir, exist_ok=True)

xai = hkl.load(AUX_DIR+'cifar10_grad.hkl')
...
xai = hkl.load(AUX_DIR+'cifar10_grad.hkl')[split_size:]
...
inv_model = ExpMI(input_shape=(32,32,3), output_shape=(32,32,3), bottleneck_size=2048, with_conf=True, num_of_classes=10)
...
loss_object = tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True)
optimizer = tf.keras.optimizers.Adam(0.0001, beta_1=0.5, beta_2=0.999)
eval_model = tf.keras.models.load_model(MODELS_DIR+'eval/cifar10', compile=False)
...
model_path = MODELS_DIR+'inv/cifar10_expmi_grad'
...
path_to_model = MODELS_DIR+'inv/cifar10_predmi'
...
if os.path.exists(path_to_model):
    ...
    print("(no existing pretrained model!)")
    # create an instance of the model
    inv_model = predmi_cifar10(num_of_classes=10)
    inv_model.compile(optimizer=optimizer, loss=loss_object)
